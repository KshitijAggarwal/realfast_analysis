...
def refine(miniSDMlist, workdir, preffileloc, gainpath = '/home/mchammer/evladata/telcal/', 
           search_sigma = 7, refine = True, classify = True, dm = 350, dm_frac=0.2, dm_steps = 100):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--search_sigma', help='Sigma of the image for search', required=False, type=int, default=7)
    ...
    refine(miniSDMlist=glob.glob(args.sdm_path), workdir=args.workdir, preffileloc=args.preffileloc, 
           ...
           classify = args.classify, dm = args.DM, dm_frac=args.DM_fraction, dm_steps = args.DM_steps)
