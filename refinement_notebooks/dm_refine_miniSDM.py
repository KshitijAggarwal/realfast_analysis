...
def refine(miniSDMlist, workdir, preffileloc, gainpath = '/home/mchammer/evladata/telcal/', 
           search_sigma = 7, refine = True, classify = True, dm = 350, dm_frac=0.2, dm_steps = 100, devicenum = '0', band='l'):
    ...
    for sdm in miniSDMlist:
        ...
        logger.info('Searching for the gainfile {0} in {1}'.format(gainname, gainpath))
    ...
    for index, sdm in enumerate(miniSDMlist):
        ...
        try:
                st = state.State(sdmfile=sdm, sdmscan=1, inprefs=prefs, preffile = preffileloc, name=name)
                cc = pipeline.pipeline_scan(st)
        except AssertionError as e:
                logger.exception('BDF not found for {0}, moving on to next sdm'.format(sdm)) 
                    ...
                    if cds:
                        for cd in cds:
                            if classify:
                                ...
                                logger.info('FETCH FRB Probability of the candidate {0} is {1}'.format(cd.candid, payload))
                            if refine:
                                logger.info('Generating Refinement plots')
                                cd_refine(cd, save=True, outdir = workdir)
                    else:
                        logger.info('No candidate was found in cc: {0}'.format(cc))
                        
                    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine DM refinement of rfpipe candidates", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument('-d', '--devicenum', help='Enter the GPU number to run pipeline', type=int, default=0, required=False)
    parser.add_argument('-b', '--band', help='Enter the band of observation', type=str, default='l', required=False)
    ...
    refine(miniSDMlist=glob.glob(args.sdm_path), workdir=args.workdir, preffileloc=args.preffileloc, 
           ...
           classify = args.classify, dm = args.DM, dm_frac=args.DM_fraction, dm_steps = args.DM_steps, devicenum=str(args.devicenum), band=args.band)
